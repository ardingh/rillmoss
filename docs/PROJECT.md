...
**本版本按用户决定启用日常使用与自动发布。** 每日任务在全部来源、规则约束和完整校验通过后更新同一个 `rillmoss.conf` 地址。住宅节点缺失时可能回退普通 V3，部分设备与协议尚未完成验收；这些限制保留，详见[启用决定](RELEASE_DECISION.md)和[设备验收](ACCEPTANCE.md)。
...
Claude 保留完整的 19 条已审阅域名及关键词规则，以及两条官方入站 IP 规则；OpenAI 独立保留 25 条规则。两组共享条目分别保留，各自校验，不能靠另一组补齐缺失。Claude 匹配规则在 OpenAI 之前，因此共享域名优先走 Claude，包括 OpenAI 使用这些域名的请求。独立是指规则内容和节点绑定各自完整，不是按调用应用隔离同一域名的流量。
...
| [个人决定](DECISIONS.md) | 对打底的保留、替换与删减 |
| [设备验收](ACCEPTANCE.md) | 日常启用前的 Mac / iPhone 验收及实际结果 |
| [维护与恢复](MAINTENANCE.md) | 每日任务、手动更新、来源失效、回退 |
| [完整备份](BACKUP.md) | 每月 15 日日期归档、漏跑补做与离线恢复 |
| [来源说明](SOURCES.md) | 来源与引用范围 |
